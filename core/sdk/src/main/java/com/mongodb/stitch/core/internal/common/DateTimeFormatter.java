...
final class DateTimeFormatter {
  private static final DateTimeFormatter.FormatterImpl FORMATTER_IMPL;

  static {
    DateTimeFormatter.FormatterImpl dateTimeHelper;
    try {
      dateTimeHelper = loadDateTimeFormatter("DateTimeFormatter$Java8DateTimeFormatter");
    } catch (LinkageError e) {
      // this is expected if running on a release prior to Java 8: fallback to JAXB.
      dateTimeHelper = loadDateTimeFormatter("DateTimeFormatter$JaxbDateTimeFormatter");
    }

    FORMATTER_IMPL = dateTimeHelper;
  }

  private static DateTimeFormatter.FormatterImpl loadDateTimeFormatter(final String className) {
    try {
      return (DateTimeFormatter.FormatterImpl) Class.forName(className).getDeclaredConstructor().newInstance();
  ...
  static class JaxbDateTimeFormatter implements DateTimeFormatter.FormatterImpl {
  ...
  static class Java8DateTimeFormatter implements DateTimeFormatter.FormatterImpl {
