...
public final class BsonUtils extends RuntimeException {
  ...
  /**
   * Parses the provided extended JSON reader and decodes it into a T value as specified by the
  ...
  public static <T> T parseValue(final InputStream jsonStream, final Decoder<T> valueDecoder) {
    final JsonStreamReader bsonReader = new JsonStreamReader(jsonStream);
